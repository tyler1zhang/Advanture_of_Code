...
.aoc.201522part1: {.aoc.spellsTable:([name:`magicMissile`drain`shield`poison`recharge];
                                      cost:53 73 113 173 229; damage:4 2 0 3 0; heal:0 2 0 0 0;
                                      armor:0 0 7 0 0; mana:0 0 0 0 101; turn:0 0 6 6 5);
                   .aoc.cost:0Wj;
                   playerStatus:`playerHit`playerMana`playerArmor!50 500 0;
                   bossStatus:`bossHit`bossDamage!"J"$last each " " vs' x;
                   effectiveDict:(`$())!`long$();
                   simulator:{[x;l;ed;playerStatus;bossStatus]
                              gameStatus:`win`loss!00b;
                              $[`shield in key ed; playerStatus[`playerArmor]:.aoc.spellsTable[`shield][`armor]; playerStatus[`playerArmor]:0];
                              if[`poison in key ed; bossStatus[`bossHit]-:.aoc.spellsTable[`poison][`damage]];
                              if[`recharge in key ed; playerStatus[`playerMana]+:.aoc.spellsTable[`recharge][`mana]];
                              ed:ed - 1;
                              ed:(where ed=0) _ ed;
                              if[bossStatus[`bossHit]<=0;gameStatus[`win]:1b];
                              if[gameStatus[`win];:.aoc.cost::exec sum cost from .aoc.spellsTable @/: l];
                              $[playerStatus[`playerMana]<.aoc.spellsTable[x][`cost]; gameStatus[`loss]:1b; playerStatus[`playerMana]-:.aoc.spellsTable[x][`cost]];
                              if[gameStatus[`loss];:0Nj];
                              l:l,x;
                              if[.aoc.cost<exec sum cost from .aoc.spellsTable @/: l;:0Nj];
                              if[x=`magicMissile; bossStatus[`bossHit]-:.aoc.spellsTable[x][`damage]];
                              if[x=`drain; bossStatus[`bossHit]-:.aoc.spellsTable[x][`damage]; playerStatus[`playerHit]+:.aoc.spellsTable[x][`heal]];
                              if[x in `shield`poison`recharge; ed,:enlist[x]!enlist .aoc.spellsTable[x][`turn]];
                              if[bossStatus[`bossHit]<=0;gameStatus[`win]:1b];
                              if[gameStatus[`win];:.aoc.cost::exec sum cost from .aoc.spellsTable @/: l];
                              $[`shield in key ed; playerStatus[`playerArmor]:.aoc.spellsTable[`shield][`armor]; playerStatus[`playerArmor]:0];
                              if[`poison in key ed; bossStatus[`bossHit]-:.aoc.spellsTable[`poison][`damage]];
                              ...
                              if[bossStatus[`bossHit]<=0;gameStatus[`win]:1b];
                              if[gameStatus[`win];:.aoc.cost::exec sum cost from .aoc.spellsTable @/: l];
                              playerStatus[`playerHit]-:max(1;bossStatus[`bossDamage]-playerStatus[`playerArmor]);
                              if[playerStatus[`playerHit]<=0;gameStatus[`loss]:1b];
                              if[gameStatus[`loss];:0Nj];
                              .z.s[;l;ed;playerStatus;bossStatus] each `magicMissile`drain`shield`poison`recharge except where ed>1};
                   min (raze/) simulator[;`$();effectiveDict;playerStatus;bossStatus] each `magicMissile`drain`shield`poison`recharge};
.aoc.201522part2: {.aoc.spellsTable:([name:`magicMissile`drain`shield`poison`recharge];
                                      cost:53 73 113 173 229; damage:4 2 0 3 0; heal:0 2 0 0 0;
                                      armor:0 0 7 0 0; mana:0 0 0 0 101; turn:0 0 6 6 5);
                   .aoc.cost:0Wj;
                   playerStatus:`playerHit`playerMana`playerArmor!50 500 0;
